...
const GHL_ENV_KEYS = [
  ...
  'GHL_LOCATION_ID_MSL',
  'GHL_PRIVATE_INTEGRATION_TOKEN_RR',
  'GHL_LOCATION_ID_RR',
  'GHL_TOKEN',
];
...
// Royal Results — client sub-account the RTL Business Package ships from
// (co-owned; RTL objects are namespaced rtl-*/RTL to stay clear of the
// client's live pipelines/workflows). TJB stays the default tenant.
addTenant('RR', 'GHL_PRIVATE_INTEGRATION_TOKEN_RR', 'GHL_LOCATION_ID_RR');
 ...
 * falling back to the standard TJB token if the scoped var isn't set.
 *
...
export function resolveByTokenGroup(tokenGroupId) {
  ...
  // Use the scoped token if available, otherwise fall back to the tenant's base token
  if (scopedToken) {
  ...
  // Fallback: use the standard tenant token (pre-scoped-token migration path)
  const base = resolve(tenantAlias);
  return {
    ...base,
    tokenGroup: tokenGroupId,
