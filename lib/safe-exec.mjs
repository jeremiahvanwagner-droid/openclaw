  ...
  try {
    ...
    if (start >= 0) {
      try {
        ...
        // Once the envelope parses, its payloads are the WHOLE answer: an
        // empty list means the agent chose not to reply, so return '' —
        // never the raw envelope (2026-07-12: a raw envelope was emailed
        // to a lead when payloads came back empty).
        const payloads = doc.payloads || doc.result?.payloads || [];
        return payloads.map(p => p && p.text).filter(Boolean).join('\n\n');
      } catch {
