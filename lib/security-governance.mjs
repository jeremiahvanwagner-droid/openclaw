...
import { enforceGhlScope as _enforceGhlScope, getAgentPermissions, GhlScopeViolation } from "./ghl-scope-enforcer.mjs";
...
// ── GHL Scope Enforcement ──────────────────────────────────────

const GHL_SCOPE_MODE_ENV = "OPENCLAW_GHL_SCOPE_ENFORCEMENT_MODE";

function getGhlScopeEnforcementMode(config) {
  const envMode = process.env[GHL_SCOPE_MODE_ENV];
  if (envMode && ["off", "warn", "fail"].includes(envMode)) return envMode;
  return config?.settings?.ghl_scope_enforcement_mode || "fail";
}

/**
 * Enforce GHL API scope constraints for an agent.
 * Integrates with the central governance pipeline so violations
 * are logged and handled consistently with other capability checks.
 *
 * @param {{ agentId: string, resource: string, operation: string, correlationId?: string, metadata?: object }} params
 * @returns {Promise<{ allowed: boolean, mode: string }>}
 */
export async function enforceGhlScopePolicy({
  agentId,
  resource,
  operation,
  correlationId,
  ...
  const config = await loadAgentsConfig();
  const mode = getGhlScopeEnforcementMode(config || {});
  if (mode === "off") {
    return { allowed: true, mode };
  }

  try {
    const result = _enforceGhlScope(agentId, resource, operation);
    return { allowed: true, mode, ...result };
  } catch (err) {
    if (err instanceof GhlScopeViolation) {
      return handleViolation({
        mode,
        code: "GHL_SCOPE_VIOLATION",
        eventName: "security/ghl_scope_violation",
        agentId,
        correlationId,
        metadata: {
          ...metadata,
          ...
          token_group: err.tokenGroup,
          agent_permissions: getAgentPermissions(agentId),
        },
        reason: err.message,
        logMessage: "GHL scope policy blocked operation",
      });
    }
    throw err;
  }
