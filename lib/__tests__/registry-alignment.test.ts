...
describe('business-registry.json wave alignment', () => {
  ...
  it('Beyond the Veil (biz_02) is in rollout wave 1', async () => {
    const raw = await readFile('data/business-registry.json', 'utf8');
    ...
    expect(btv.rollout_wave).toBe(1);
  });

  it('Divine Path Walkers (biz_03) is in rollout wave 1', async () => {
    const raw = await readFile('data/business-registry.json', 'utf8');
    const { businesses } = JSON.parse(raw);
    ...
    expect(dpw.rollout_wave).toBe(1);
  });
